...
*nix系统总灵活多变著称，在各种头、尾情况下，都有令人惊讶的表现。  今天BigEagle主要就带大家玩一下多头和多尾的配置。<!--more-->
### X server 的多显示器支持
...
{% codeblock %}smpalyer -display :0.1{% endcodeblock %}
...
{% codeblock %} Section "ServerLayout"
 ...
 EndSection{% endcodeblock %}
好了，现在，startx~
<a href="http://bigeagle.me/wp-content/uploads/2011/04/dual_monitor.jpeg"><img class="shadow aligncenter size-medium wp-image-144" title="双屏幕的工作空间" src="http://bigeagle.me/wp-content/uploads/2011/04/dual_monitor-300x225.jpg" alt="" width="300" height="225" /></a>
...
<a href="http://bigeagle.me/wp-content/uploads/2011/04/xdmx.jpg"><img class="shadow alignleft size-medium wp-image-148" title="三台X server" src="http://bigeagle.me/wp-content/uploads/2011/04/xdmx-300x111.jpg" alt="" width="300" height="111" /></a>
...
<a href="http://bigeagle.me/wp-content/uploads/2011/04/xdmx2x2.jpg"><img class="shadow alignright size-medium wp-image-149" title="2x2" src="http://bigeagle.me/wp-content/uploads/2011/04/xdmx2x2-300x240.jpg" alt="" width="300" height="240" /></a>
首先，你需要以下硬件：
<ul>
...
{% codeblock %}  apt-get install xdmx             #Debian 系
  yum install xdmx                 #Red Hat 系
  pacman -S xorg-server-xdmx       #Archlinux{% endcodeblock %}
万事具备，只欠启动~别急，你的X server一般是工作在本地运行状态的，执行
{% codeblock %}ps ax|gerp X{% endcodeblock %}
...
{% codeblock %}DisallowTCP = false{% endcodeblock %}
即可，对于修改了daemon.conf的同学，这一句要加到[security]项里。
...
{% codeblock %}xhost + control_node_ip{% endcodeblock %}
这里的control_node_ip是主机的IP地址，我们假定为192.168.0.1，为方便叙述，再假定从机的IP地址为192.168.0.101。这句命令的意思是 赋予主机访问从机X server的权限。

接着，到主机处，运行一个X server，不管怎样，需要一个X终端，化腐朽为神奇的时刻就要到来，运行命令：
{% codeblock %}startx `which twm` -- /usr/bin/Xdmx :1 -display 192.168.0.1:0 \
          -display 192.168.0.101:0 -ignorebadfontpaths +xinerama -noglxproxy{% endcodeblock %}
...
在终端中输入：
{% codeblock %}xinput --list{% endcodeblock %}
你应该能看见一组硬件信息，例如BigEagle的就是：
{% codeblock %}⎡ Virtual core pointer                      	id=2	[master pointer  (3)]
...
⎣ Virtual core keyboard                    	id=3	[master keyboard (2)]
    ...
    ↳ Dell WMI hotkeys                        	id=14	[slave  keyboard (3)]{% endcodeblock %}
接下来，输入
{% codeblock %}xinput --create-master xxx{% endcodeblock %}
xxx是自己随便起的名字，这时你会看到屏幕上出现了一个新的光标，但是怎么都动不了，不要担心，这是因为还没有个这个光标加入任何新的设备。下面，我们输入：
{% codeblock %}xinput --reattach 10 "xxx pointer"{% endcodeblock %}
...
{% codeblock %}xinput --reattach 9 "xxx keyboard"{% endcodeblock %}
将Dell USB Keyboard加入xxx光标指向的键盘，这样，两个键盘也能独立工作了。

想要取消多光标的设置，只需要：
{% codeblock %}xinput --remove-master xxx{% endcodeblock %}
即可。
### 写在最后
