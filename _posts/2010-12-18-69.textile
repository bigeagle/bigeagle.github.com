...
<a href="http://bigeagle.me/wp-content/uploads/2010/12/twittperator.png"><img class="aligncenter size-medium wp-image-70 shadow" title="twittperator" src="http://bigeagle.me/wp-content/uploads/2010/12/twittperator-300x188.png" alt="" width="300" height="188" /></a>一下子看不出来这是Firefox吧！
<!--more-->
### 安装
...
{% codeblock %}:tw -getPIN{% endcodeblock %}
...
{% codeblock %}:tw!@username{% endcodeblock %}
...
{% codeblock %}:tw RT @username#ID{% endcodeblock %}
官方RT。其中的username#ID是可以通过tab补全的，范围和用法同上。

{% codeblock %}:tw &lt;COMMENT&gt;  RT @username#ID{% endcodeblock %}
非官方RT。补全同上。

{% codeblock %}:tw @username#ID{% endcodeblock %}
针对一个推回复。补全当然同上。

{% codeblock %}:tw!delete{StatusID}{% endcodeblock %}
删除一个Tweet，这个木有补全

{% codeblock %}:tw!+@username#ID
:tw!-@username#ID{% endcodeblock %}
加入收藏/取消收藏。补全自然还是同上。

{% codeblock %}:tw!?{% endcodeblock %}
搜索关键词。搜索和缓存无关。

{% codeblock %}:tw!/{% endcodeblock %}&lt;URL&gt;
打开推上的URL。补全跟着缓存走，补全的关键词是推的内容。

<span style="text-decoration: line-through;">{% codeblock %}:tw!info{% endcodeblock %}
查看一条推的详细信息。大到内容，发件人，时间，小到URL，头像地址，背景颜色，回复对象，用户名详细信息等等。</span>
## 最后
